...
export const TextContent: React.FC<ITextContent> = (props) => {
  return (
    <div className='flex items-center gap-6 md:gap-0 flex-col md:justify-between pb-2 md:py-4 md:h-full'>
        <h3 className='uppercase font-bold text-center'>{props.header}</h3>
        <p className='text-primary-500 text-sm max-w-[480px] w-[80%]'>{props.description}</p>
        <div className='flex flex-wrap gap-3'>
          { props.stack.map((stack, indx) => <Stack key={indx}>{stack}</Stack>) }
        </div>
        <div className='flex items-center gap-x-12'>
          <ButtonLink text='github' img={
